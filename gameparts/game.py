...
class Game:
    def __init__(self):
        ...
        self.spell_animations = {
            "Fireball": [pygame.transform.scale(pygame.image.load(f"visuals/fireballanim/tile{i}.png").convert_alpha(), (200, 200)) for i in range(0, 17)],
    ...
    def draw_health_bars(self):
        for i, player in enumerate(self.players):
            ...
            mana_bar_length = 150
            mana_bar_height = 15
            mana_ratio = player.mana / 100
            mana_bar_position = (94 + i * 400, 90)
            mana_bar_position_img = (70 + i * 400, 75)
            pygame.draw.rect(self.screen, (0, 0, 255), (mana_bar_position[0], mana_bar_position[1], mana_bar_length, mana_bar_height))
    ...
    def play_spell_animation(self, spell_name, target_position):
        for frame in self.spell_animations[spell_name]:
            ...
            self.draw_health_bars()
            self.draw_turn_info()
            self.screen.blit(frame, target_position)
            pygame.display.flip()
